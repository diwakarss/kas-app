...
import { formatDateValue, humanizeDateStrings } from '../../src/lib/format-dates';
...
describe('humanizeDateStrings', () => {
  ...
  test('leaves non-date text alone', () => {
    expect(humanizeDateStrings('Asha Kumar — Piano')).toBe('Asha Kumar — Piano');
    expect(humanizeDateStrings('')).toBe('');
  });

  test('handles multiple ISO strings in one input', () => {
    const out = humanizeDateStrings('From 2026-04-28T09:00:00Z to 2026-04-28T10:00:00Z');
