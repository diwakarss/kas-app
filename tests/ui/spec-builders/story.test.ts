...
import type { StoryData, TimelineEventData } from '../../../src/hooks/useStoryData';

function makeMockEvent(overrides: Partial<TimelineEventData> = {}): TimelineEventData {
  return {
    id: 1,
    sourceEntity: 'class',
    eventType: 'completed',
...
function makeMockStoryData(overrides: Partial<StoryData> = {}): StoryData {
  return {
    ...
    comingUp: [
      { display: '2026-04-17 - Theory', id: 10, sortDate: '2026-04-17T10:00:00Z' },
    ],
    events: [
      makeMockEvent(),
      makeMockEvent({ id: 2, display: '2026-04-14 - Rescheduled', iconColor: '#5B8BA4' }),
    ],
...
describe('buildStorySpec', () => {
  ...
  test('handles empty events gracefully', () => {
    const spec = buildStorySpec(makeMockStoryData({ events: [], totalLoaded: 0 }));
    const result = validateSpec(spec);
    expect(result.valid).toBe(true);
  ...
  test('includes warnings when present', () => {
    const spec = buildStorySpec(
      makeMockStoryData({
        warnings: [{ message: 'Payment overdue', severity: 'warning', locations: ['story'] }] as any,
      }),
    );
