...
function ent(overrides: Partial<Entity> & { name: string }): Entity {
...
describe('injectMissingDomainEntities', () => {
  test('injects Property for cleaning vertical when missing', () => {
    const entities = [
      ent({ name: 'Client', fields: [{ name: 'name', type: 'text' }, { name: 'email', type: 'email' }] }),
      ent({
        name: 'Job',
        fields: [
        ...
        relationships: [{ target: 'Client', type: 'belongs_to', foreign_key: 'client_id', display_in_story: true }],
      }),
    ...
    expect(property!.relationships.some(r => r.target === 'Client')).toBe(true);

    const job = out.find(e => e.name === 'Job')!;
    expect(job.fields.some(f => f.name === 'property_id')).toBe(true);
    expect(job.relationships.some(r => r.target === 'Property' && r.foreign_key === 'property_id')).toBe(true);
  });
  ...
  test('injects Property using business name hint when entity names lack the keyword', () => {
    const entities = [
      ...
      ent({
        name: 'Job',
        fields: [{ name: 'scheduled_at', type: 'datetime' }, { name: 'client_id', type: 'number' }],
        relationships: [{ target: 'Client', type: 'belongs_to', foreign_key: 'client_id', display_in_story: true }],
      }),
  ...
  test('injects Pet for dog-walking vertical', () => {
    const entities = [
      ent({ name: 'Client', fields: [{ name: 'name', type: 'text' }, { name: 'phone', type: 'phone' }] }),
      ent({
        name: 'Walk',
        fields: [{ name: 'scheduled_time', type: 'datetime' }, { name: 'client_id', type: 'number' }],
        relationships: [{ target: 'Client', type: 'belongs_to', foreign_key: 'client_id', display_in_story: true }],
      }),
  ...
  test('does not inject Property when one already exists (case-insensitive)', () => {
    const entities = [
      ...
      ent({ name: 'Job', fields: [{ name: 'scheduled_at', type: 'datetime' }] }),
    ];
