...
export function useStoryData(entityType: string, entityId: number, page: number = 0): StoryData | null {
  ...
  return useMemo(() => {
    ...
    const events: TimelineEventData[] = rawEvents.map(row => {
      ...
      const srcEntityDef = spec.entities.find(e => e.name.toLowerCase() === row._source_entity);
      if (srcEntityDef) {
      ...
      const display = eventConfig
        ...
        : `${row._event_type}`;
      const iconColor = eventConfig ? resolveColor(eventConfig.icon_color) : resolveColor('mist');
