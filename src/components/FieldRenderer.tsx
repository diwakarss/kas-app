...
export default function FieldRenderer({
  ...
  switch (field.type) {
    ...
    case "duration":
      return (
        <TextInputField
          ...
          placeholder={placeholder}
          prefix={
            prefix || (field.type === "currency" ? field.prefix : undefined)
          }
      ...
      return (
        <View className="items-start py-2">
          <Switch
            ...
            trackColor={{ false: colors.mist, true: colors.bloom }}
            thumbColor={colors.dawn}
          />
        </View>
      );
    case "date":
    ...
    case "datetime":
      return (
        <DatePickerField value={value} onChange={onChange} mode="datetime" />
      );
    case "time":
      ...
      return (
        <View className="py-4">
          <Text className="font-inter text-sm text-mist italic">
            Image upload coming in a future update
          </Text>
        </View>
      );
    default:
...
function TextInputField({
  ...
  return (
    <View className="flex-row items-center">
      {prefix ? (
        <Text className="font-inter text-mist mr-2" style={{ fontSize: 20 }}>
          {prefix}
        </Text>
      ) : null}
      <TextInput
        ...
        style={
          {
            ...
            borderBottomWidth: 2,
            borderBottomColor: colors.mist + "40",
            paddingVertical: 12,
            color: colors.clay,
            ...(multiline
              ? { minHeight: 120, textAlignVertical: "top" as const }
              : {}),
            outlineStyle: "none",
          } as any
        }
      ...
      />
      {suffix ? (
        <Text className="font-inter text-mist ml-2" style={{ fontSize: 16 }}>
          {suffix}
        </Text>
      ) : null}
    </View>
        ...
        {options.map((opt) => (
          <Pressable
            ...
            style={{
              ...
              borderWidth: 1.5,
              borderColor: value === opt ? colors.ember : colors.mist + "50",
            }}
          >
            <Text
              className="font-inter-medium"
              style={{ color: value === opt ? colors.dawn : colors.clay }}
            >
              {opt}
            </Text>
          </Pressable>
      ...
      {Platform.OS === "android" && !show && (
        ...
        >
          <Text
            ...
            style={{ fontSize: 24, color: value ? colors.clay : colors.mist }}
          >
            {value || `Select ${mode}`}
          </Text>
        </Pressable>
