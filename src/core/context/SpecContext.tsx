...
import { loadSpec } from '../../engines/spec-loader';
import { generateDDL } from '../../engines/schema-engine';
...
/**
 * Shared initialization logic used by both NativeSpecProvider and WebSpecProvider.
 * Takes a platform-specific DatabaseAdapter and returns the full context value.
 */
export function initializeSpec(adapter: DatabaseAdapter): SpecContextValue {
  console.log('[SpecContext] Starting initialization...');
  ...
  if (!result.success) {
    console.log('[SpecContext] Spec validation failed:', result.errors);
    return {
      spec: null,
      db: null,
      crud: null,
  ...
  console.log('[SpecContext] Spec loaded:', validSpec.meta.name);

  // 2. Initialize schema (idempotent — IF NOT EXISTS)
  const ddlStatements = generateDDL(validSpec);
  const isWeb = Platform.OS === 'web';
  console.log('[SpecContext] DDL statements:', ddlStatements.length, isWeb ? '(web — skipping FTS5)' : '');
  for (let i = 0; i < ddlStatements.length; i++) {
    ...
    try {
      ...
      console.log(`[SpecContext] DDL[${i}] failed:`, ddlErr.message);
    }
  }
  console.log('[SpecContext] Schema initialized');
  ...
  console.log(`[SpecContext] ${firstTable} count:`, count?.cnt);
  if (count && count.cnt === 0) {
    ...
    console.log('[SpecContext] Seed data inserted');
  }

  console.log('[SpecContext] Initialization complete');
  return {
    spec: validSpec,
    db: adapter,
    crud: crudService,
...
// ──────────────────────────────────────────
// Legacy SpecProvider — used by App.tsx on native
// Kept for backward compatibility; new code uses NativeSpecProvider/WebSpecProvider
// ──────────────────────────────────────────
...
export function SpecProvider({ children }: SpecProviderProps) {
  // Dynamic import: only use NativeSpecProvider or WebSpecProvider
  if (Platform.OS === 'web') {
